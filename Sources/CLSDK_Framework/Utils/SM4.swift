...
public class Sm4Impl {
  ...
  func setKeyString(keyString: String, ivString: String) {
    ...
    if key.count != 16 {
      ...
      let hashKey = getSM3().update(keyString).finalize().getHash()
      let sm3Bytes = stringToBytes(text: hashKey)
      for i in 0..<min(sm3Bytes.count, 16) {
        key[i] = sm3Bytes[i]
      }
    }
    ...
    if iv.count != 16 {
      ...
      let hashIv = getSM3().update(ivString).finalize().getHash()
  ...
  func initKey(key: [UInt8], iv: [UInt8]) {
    ...
    for i in 0..<(key.count / 4) {
      MK[i] =
        (UInt32(key[offset]) & 0xff) << 24 | (UInt32(key[offset + 1]) & 0xff) << 16
        | (UInt32(key[offset + 2]) & 0xff) << 8 | UInt32(key[offset + 3]) & 0xff
      offset += 4
    }
  ...
  func doCrypt(input: [UInt8], cbcIV: [UInt8], encrypt: Bool) -> String {
    ...
    for _ in 0..<(input.count / 16) {
      ...
      for i in 0..<4 {
        if encrypt {
          X[i] =
            ((UInt32(input[offset]) ^ UInt32(cbcIV[i * 4])) & 0xff) << 24
            | ((UInt32(input[offset + 1]) ^ UInt32(cbcIV[i * 4 + 1])) & 0xff) << 16
            | ((UInt32(input[offset + 2]) ^ UInt32(cbcIV[i * 4 + 2])) & 0xff) << 8
            | (UInt32(input[offset + 3]) ^ UInt32(cbcIV[i * 4 + 3])) & 0xff
        } else {
          X[i] =
            (UInt32(input[offset]) & 0xff) << 24 | (UInt32(input[offset + 1]) & 0xff) << 16
            | (UInt32(input[offset + 2]) & 0xff) << 8 | UInt32(input[offset + 3]) & 0xff
        }
        offset += 4
      }
      for i in 0..<32 {
        let roundKey = rk[i]
        X[i + 4] = F(x0: X[i], x1: X[i + 1], x2: X[i + 2], x3: X[i + 3], rk: encrypt ? roundKey : rk[31 - i])
      }
      ...
      var index = 0
      for i in 0..<4 {
        XBO.append(limitToByte(i: Int((XRO[i] >> 24) & 0xff)))
        XBO.append(limitToByte(i: Int((XRO[i] >> 16) & 0xff)))
        XBO.append(limitToByte(i: Int((XRO[i] >> 8) & 0xff)))
        XBO.append(limitToByte(i: Int(XRO[i] & 0xff)))
        index += 1
      }
      if encrypt {
        // 加密时更新IV为当前块的密文
    ...
    if encrypt {
      ...
      let bs = out
      // 严格检查PKCS#7填充
      let pad = Int(bs[bs.count - 1])
      if pad > 0 && pad <= 16 && bs[(bs.count - pad)..<bs.count].allSatisfy({ $0 == pad }) {
        return bytesToUTF8String(bytes: Array(bs[0..<(bs.count - pad)]))
      }
      return bytesToUTF8String(bytes: bs)
    }
  }
